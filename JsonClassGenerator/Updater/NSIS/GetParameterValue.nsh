...
Function GetParameters
  ...
  get:
	...
	#	MessageBox MB_OK "GetParameters: '$R0'"
...
next:
	...
	#MessageBox MB_OK "Length of search string is: >$R2<"
	StrCpy $R0 $R4 "" $R2
 	#MessageBox MB_OK "CmdLine from parameter: >$R4<"
  ...
  StrCpy $R5 $R0 1
  StrCmp $R5 '"' quoted unquoted
  quoted:
 	#MessageBox MB_OK "Quoted"
	StrCpy $R0 $R0 "" 1
	Goto endif
  unquoted:
	StrCpy $R5 " "
 ...
 #MessageBox MB_OK "Marker: >$R5< raw: >$R0<"
 
	StrCpy $R4 0
 
	loop:
		StrCpy $R2 $R0 1 $R4
		StrCmp $R2 $R5 endloop
		StrCmp $R2 '' endloop
		IntOp $R4 $R4 + 1
	Goto loop
	endloop:
 ...
 #MessageBox MB_OK "Value: >$R0<"
  /*
  Push ' '            ; search for either the first space ' ', or the first
                      ; quote '"'
